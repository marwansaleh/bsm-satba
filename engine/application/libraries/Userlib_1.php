...
class Userlib extends Library {
    ...
    private $_table_roles = 'ref_auth_roles';
    ...
    public function get_user_privileges($user_id){
        ...
        foreach($all_roles as $role){
            if ($this->is_admin($group_id)){
                ...
                $role->granted = isset($user_access[$role->role_id]) ? $user_access[$role->role_id] : (isset($group_access[$role->role_id]) ? $group_access[$role->role_id] : FALSE);
            }
    ...
    private function _set_user_access_session($user_id, $group_id){
        ...
        if (!$this->is_admin($group_id)){
            ...
            foreach ($this->_get_users_access($user_id) as $u_role){
                $user_roles [$u_role->role_id] = $u_role->granted == 1?TRUE:FALSE;
            }
        }
        ...
        foreach ($this->_get_all_roles() as $role){
            if ($this->is_admin($group_id)){
                $access_roles[$this->_prefix_session_access . $role->name] = TRUE;
            }else{
                $access_roles[$this->_prefix_session_access . $role->name] = isset($user_roles[$role->role_id]) ? $user_roles[$role->role_id] : (isset($group_roles[$role->role_id]) ? $group_roles[$role->role_id] : FALSE);
            }
        }
