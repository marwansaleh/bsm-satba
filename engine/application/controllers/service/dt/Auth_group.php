...
/**
 * Description of Auth_group
 *
 * @author marwansaleh 1:31:58 PM
 */
class Auth_group extends REST_Api {
    private $_remap_fields = array(
        ...
        'removable'         => 'removable',
        'removable_label'   => 'removable_label',
        'created'           => 'created',
    ...
    function index_get(){
        ...
        $totalRecords = $this->auth_group_m->get_count();
        ...
        //get filtered count
        $totalFiltered = $this->auth_group_m->get_count();
        ...
        if ($totalRecords > 0){
            ...
            $query_result = $this->auth_group_m->get();
            if ($query_result){
                $items = array();
    ...
    private function _proccess_item($item=NULL){
        if ($item){
            $item->removable_label = $item->removable==1 ? 'Yes' : 'No';
            $item->user_count = $this->auth_user_m->get_count(array('group_id'=>$item->id));
